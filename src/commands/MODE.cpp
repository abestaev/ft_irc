...
int Commands::cmd_mode(const Message &msg, Client &sender)
{
    ...
    if (!target.empty() && (target[0] == '#' || target[0] == '&' || target[0] == '!' || target[0] == '+'))
    {
        ...
        if (!ch)
            return send_error(sender, 403, target + " :No such channel"), -1;
        if (msg.getParamCount() == 1)
        {
            std::string modes = "+";
            if (ch->isInviteOnly())
            ...
            if (ch->hasKey())
                reply += " *";
            if (ch->getUserLimit() > 0)
                reply += " " + int_to_string(ch->getUserLimit());
            send_reply(sender, 324, reply);
            return 0;
        }
        if (!ch->isOperator(sender))
        {
            send_error(sender, ERR_CHANOPRIVSNEEDED, target + " :You're not channel operator");
            return -1;
        }
        ...
        for (size_t i = 0; i < modespec.size(); ++i)
        {
            ...
            if (m == 'k')
            {
                if (adding)
                {
                    if ((size_t)paramIndex > msg.getParamCount() - 1)
                    {
                        send_error(sender, ERR_NEEDMOREPARAMS, "MODE :Not enough parameters");
                        return -1;
                    }
            ...
            if (m == 'o')
            {
                ...
                {
                    if (clients[ci].fd != -1 && clients[ci].nick == nick)
                    {
                        targetClient = clients[ci];
                        found = true;
                        break;
                    }
                }
                if (!found || !ch->has_client(targetClient))
                {
                    send_error(sender, ERR_NOTONCHANNEL, nick + " " + target + " :They aren't on that channel");
                    continue;
                }
            ...
            send_error(sender, ERR_UNKNOWNMODE, std::string(1, m) + " :is unknown mode char to me");
        }
    ...
    if (msg.getParamCount() == 1)
    {
        send_reply(sender, RPL_UMODEIS, sender.getUserModes());
        return 0;
    }
    std::string modespec = msg.getParams()[1];
    bool adding = true;
    ...
    for (size_t i = 0; i < modespec.size(); ++i)
    {
        ...
        send_error(sender, ERR_UMODEUNKNOWNFLAG, std::string(1, m) + " :Unknown MODE flag");
    }
    if (!appliedSpec.empty())
    {
        std::string prefix = sender.nick;
