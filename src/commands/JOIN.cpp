...
int Commands::cmd_join(const Message& msg, Client& sender)
{
    ...
    for (size_t idx = 0; idx < channels.size(); ++idx) {
        ...
        if (ch->getUserLimit() > 0 && ch->get_user_count() >= ch->getUserLimit()) { send_error(sender, 471, channel_name + " :Cannot join channel (+l)"); continue; }
        if (ch->isInviteOnly() && !ch->isNickInvited(sender.nick)) { send_error(sender, 473, channel_name + " :Cannot join channel (+i)"); continue; }
        if (ch->hasKey() && ch->getKey() != provided_key) { send_error(sender, 475, channel_name + " :Cannot join channel (+k)"); continue; }
