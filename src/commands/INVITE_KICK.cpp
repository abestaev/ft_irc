...
{
    if (msg.getParamCount() < 2) { send_error(sender, 461, "INVITE :Not enough parameters"); return -1; }
    ...
    Client target; bool found = false;
    for (int i = 0; i < _server->getMaxClients(); i++) { if (clients[i].fd != -1 && clients[i].nick == nick) { target = clients[i]; found = true; break; } }
...
{
    if (msg.getParamCount() < 2) { send_error(sender, 461, "KICK :Not enough parameters"); return -1; }
    ...
    std::string prefix = sender.nick; if (!sender.username.empty() && !sender.hostname.empty()) prefix += "!" + sender.username + "@" + sender.hostname;
    std::string wire = ":" + prefix + " KICK " + channel_name + " " + nick; if (!reason.empty()) wire += " :" + reason; wire += "\r\n";
    ch->broadcast(wire, -1);
